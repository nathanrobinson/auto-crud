...
public static class Extensions
{
        ...
        where TChangeTrackingEntity : ChangeTrackingEntity<TKey, TEntity>, new()
    {
        ...
        // ChangeTrackingEntity<TKey,TEntity> type and shared with the Mongo change-tracking
        // implementation and the Web read controller - it can only be wired up when that's
        // actually the row type in use. EF Core does not expose a custom row type's CLR base
        // class as a separately queryable entity type once only the derived type is mapped, so
        // registering these against a custom TChangeTrackingEntity would fail at query time.
